...
func (r *Resource) EnsureCreated(ctx context.Context, obj interface{}) error {
	...
	if apierrors.IsAlreadyExists(err) {
		r.logger.Debugf(ctx, "updating configmap %s/%s already exists", configmap.Namespace, configmap.Name)
		err = r.k8sClient.CtrlClient().Update(ctx, configmap)
		if err != nil {
			return microerror.Mask(err)
		}
		r.logger.Debugf(ctx, "updated configmap %s/%s already exists", configmap.Namespace, configmap.Name)
	} else if err != nil {
		return microerror.Mask(err)
	} else {
	...
	if apierrors.IsAlreadyExists(err) {
		r.logger.Debugf(ctx, "updating secret %s/%s already exists", secret.Namespace, secret.Name)
		err = r.k8sClient.CtrlClient().Update(ctx, secret)
		if err != nil {
			return microerror.Mask(err)
		}
		r.logger.Debugf(ctx, "updated secret %s/%s already exists", secret.Namespace, secret.Name)
	} else if err != nil {
		return microerror.Mask(err)
	} else {
